...
// App.DATASETS = ['tree', 'octopus', 'umbrella','snowman'];
...
App.MainState.prototype = {
	...
	*/
	preload : function(){
		this.game.load.image('imgBack', '../assets/IMG_BG.png'); //background
		this.game.load.image('imgDisable', '../assets/disable_img.png'); //disable draw pad 
		
		this.game.load.image('btnTrain', '../assets/btn_trainmore.png'); //buttons 
		this.game.load.image('btnTest', '../assets/btn_more.png');
		this.game.load.image('btnClear', '../assets/btn_clear.png');
		
		this.load.bitmapFont('fntBlackChars', '../assets/fnt_black_chars.png', '../assets/fnt_black_chars.fnt'); //text
	},
	...
	update : function(){
		switch(this.mode){
			...
			case this.MODE_OPEN_FILE:
				var fileName = App.DATASETS[this.dataset] + '.bin'; //import data ke dlm
			...
			case this.MODE_DO_TRAIN:
				if (this.cnn.isTrainCompleted){
					this.ui.showStatusBar("Training selesai. Memprediksi sample...");
